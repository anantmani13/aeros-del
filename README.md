...
| `MAPTILER_KEY` | maptiler.com | Premium basemap (optional, OSM/Esri keyless by default) |
| `CARTO_KEY` | carto.com/basemaps/apikey | Restores CARTO basemap look (optional, free) |
...
- API keys (all optional — missing keys = demo fallback, nothing crashes): `OPENAQ_API_KEY` (live PM), `NASA_FIRMS_API_KEY` (fires), `GEMINI_API_KEY` (LLM advisories), `MAPTILER_KEY` (premium basemap; keyless OSM/Esri default otherwise), `CARTO_KEY` (restores CARTO look — CARTO raster watermarks "API key required" without it)
